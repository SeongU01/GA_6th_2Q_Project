...
public:
	void SetRange(Vector3 position, D2D1_SIZE_F size) {_buttonRange = { position.x-(size.width*0.5f), position.y - (size.width * 0.5f),position.x + (size.width * 0.5f),position.y +( size.height * 0.5f)}; }
