...
int BossEnemyRealMeteorSlash::Update(const float& deltaTime)
{
...
int BossEnemyRealMeteorSlash::LateUpdate(const float& deltaTime)
{
	 if (!_isStateOn)
		ShowInfo();
...
void BossEnemyRealMeteorSlash::OnStart()
{
...
void BossEnemyRealMeteorSlash::OnExit()
{
	CloseInfo();
}

void BossEnemyRealMeteorSlash::ShowInfo()
{
	__super::ShowInfo();
	if (_pAnimation->IsCurrAnimation(L"Idle"))
	{
...
void BossEnemyRealMeteorSlash::CloseInfo()
{
	__super::CloseInfo();
}

void BossEnemyRealMeteorSlash::Attack()
{
...
BossEnemyRealMeteorSlash* BossEnemyRealMeteorSlash::Create(BossEnemyScript* pScript)
{
	BossEnemyRealMeteorSlash* pInstance = new BossEnemyRealMeteorSlash;
	pInstance->BossEnemyState::Initialize(pScript);
	Engine::Animation::FrameEvent frameEvent;
	frameEvent.activeFrame = 6;
	frameEvent.animation = L"RealMeteorSlash";
